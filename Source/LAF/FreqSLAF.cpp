...
void FreqSliderLookAndFeel::drawLinearSlider(juce::Graphics& g, int x, int y, int width, int height, float sliderPos, float minSliderPos, float maxSliderPos, const juce::Slider::SliderStyle /*style*/, juce::Slider& slider)
{
    ...
    double normalizedValue = (slider.getValue() - slider.getMinimum()) / static_cast<double>(slider.getMaximum());

    //=============================================================================
    // Creates Background Track for Slider
    //=============================================================================
    ...
    {
        g.setColour(mainSliderColour_.withSaturation(normalizedValue * 0.9f).darker(0.075f));
    }
    else
    {
    ...
    if (mainSliderColour_ != C1::grey && mainSliderColour_ != C2::grey && mainSliderColour_ != C3::grey)
    {
        // g.setColour (mainSliderColour_.withSaturation (normalizedValue * 0.7f).darker (0.075f));
    }
    else
    {
        // Set a fallback color for grey
    ...
    if (mainSliderColour_ != C1::grey && mainSliderColour_ != C2::grey && mainSliderColour_ != C3::grey)
    {
        g.setColour(mainSliderColour_.withSaturation(normalizedValue * 0.7f));
    }
    else
    {
        // Set a fallback color for grey
